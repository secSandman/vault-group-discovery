...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: python ad-group-namespace-discovery.py <Vault_Token|-> <target_url>")
        sys.exit(1)
    
    token = sys.argv[1]
    target = sys.argv[2].rstrip('/')  # Remove any trailing slash

    # If token is '-' then prompt securely for token
    if token == '-':
